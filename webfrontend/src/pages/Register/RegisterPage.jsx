  ...
  return (
    ...
    >
      <div className="max-w-[520px] w-full p-6 sm:p-10 bg-white/10 rounded-[18px] shadow-[0_8px_32px_0_rgba(207,207,211,0.6)] backdrop-blur-[16px] backdrop-saturate-[180%] border-[1.5px] border-[#e0e7ef]">
        <h2 className="text-xl sm:text-2xl font-bold mb-6 text-center text-gray-800">注册</h2>
        <form onSubmit={handleSubmit}>
          ...
          {success && <div className="text-green-500 mb-2 text-sm sm:text-base">{success}</div>}
          <button 
            type="submit" 
            disabled={loading}
        ...
        <div className="mt-4 text-center">
          <span className="text-gray-700 text-sm sm:text-base">已有账号？ </span>
          <Link to="/login" className="text-[#646cff] no-underline hover:text-[#4f8cff] transition-colors duration-200 text-sm sm:text-base">去登录</Link>
        </div>
      </div>
    </div>
  );
